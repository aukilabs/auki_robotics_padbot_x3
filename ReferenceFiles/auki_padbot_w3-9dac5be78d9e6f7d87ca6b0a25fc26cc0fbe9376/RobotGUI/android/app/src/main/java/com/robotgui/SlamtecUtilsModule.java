...
public class SlamtecUtilsModule extends ReactContextBaseJavaModule {
    ...
    public void checkConnectionSdk(Promise promise) {
        executorService.execute(() -> {
            SlamwareSdpPlatform platform = null;
            try {
                WritableMap response = Arguments.createMap();
    ...
    // Add helper method to log to debug_log.txt
    private void logToFile(String message) {
        try {
            ...
            Log.e(TAG, "Error writing to debug log file: " + e.getMessage(), e);
        }
    }
    ...
    public void downloadYamlFile(Promise promise) {
        executorService.execute(() -> {
            try {
                ...
                if (connection.getResponseCode() == HttpURLConnection.HTTP_OK) {
                    // Create X3-Prep directory in home
                    File homeDir = new File(System.getProperty("user.home"));
                    File cactusDir = new File(homeDir, "X3-Prep");
                    if (!cactusDir.exists()) {
                        cactusDir.mkdirs();
                    }
        ...
        executorService.execute(() -> {
            ...
            try {
                ...
                try {
                    ...
                    map = mapHelper.loadFile(inputFilePath);
                    if (map == null) {
                ...
                if (map != null) {
                    try {
                        ...
                        Log.e(TAG, "Error cleaning up map resources: " + e.getMessage(), e);
                        logToFile("Error cleaning up map resources: " + e.getMessage());
                    }
                }
                ...
                if (platform != null) {
                    try {
                        platform.disconnect();
                        Log.d(TAG, "Disconnected from platform");
                        logToFile("Disconnected from platform");
                    } catch (Exception e) {
                        Log.e(TAG, "Error disconnecting from platform: " + e.getMessage(), e);
                        logToFile("Error disconnecting from platform: " + e.getMessage());
                        // Don't rethrow, we're already in finally
                    }
                }
            }
        });
            ...
            try {
                ...
                if (platform != null) {
                    ...
                    if (retryCount >= maxRetries) {
                        ...
                        logToFile("Home navigation timeout - assuming success after " + maxRetries + " checks");
                    }
